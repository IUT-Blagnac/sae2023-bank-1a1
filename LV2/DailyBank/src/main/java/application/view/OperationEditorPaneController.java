		...
		switch (this.categorieOperation) {
			case DEBIT:
				...
				if (this.dailyBankState.isChefDAgence()
					...
					soldepresent.setContentText(
							"Le montant de débit dépasse le découvert autorisé\n Êtes-vous sûr de vouloir continuer ce débit ?");
					...
					if (result.isPresent() && result.get() == ouiButton) {
						...
						this.operationResultat = new Operation(-1, montant, null, null, this.compteEdite.idNumCli,
								typeOp);
					} else {
						return;
					}
			...
			case TRANSFERT:
				...
				this.operationResultat = new OperationTransfert(-1, montant, null, null, this.compteEdite.idNumCli,
						typeOp, NumCompteDestinataire);
				this.primaryStage.close();
				break;
		}
	}
