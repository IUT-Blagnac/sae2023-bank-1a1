...
^|Gestion_T_CC_Cree.1  ^|Rentrer un montant 50 dans le champ "Decouvert Autoriser" et Rentrer un montant 10 dans le champ "solde(premier depot)" ^|Le découvert autorisé est -50euros le solde du compte est de 10. On a créé une nouveau compte dans la liste des compte avec son solde et son découvert autorisé ^| Le compte client n'est pas desactivé
 ^|OK
